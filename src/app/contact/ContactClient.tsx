...
function ContactFormSection() {
  ...
  const { cartItems, openDrawer, clearCart } = useCart();
  ...
  return (
    <section className="grid gap-12 lg:grid-cols-[0.9fr_1.1fr] items-start relative">
      ...
      <div className="space-y-8">
        ...
        <div className="space-y-6 text-xs tracking-wider">
          ...
          <div className="flex gap-4 items-start border-l border-[#b8924a]/30 pl-4 py-1">
            ...
            <div className="space-y-1">
              ...
              <a 
                ...
                target="_blank" 
                rel="noreferrer" 
                className="text-[#faf8f4] hover:text-[#d4a96a] transition"
              >
                Plot No. C-15, Main Road, Block C, VIP Industrial Park, <br />
                Palsana, Surat, Gujarat 394315, India
              </a>
            </div>
          </div>
          ...
          <div className="flex gap-4 items-start border-l border-[#b8924a]/30 pl-4 py-1">
            ...
            <div className="space-y-1">
              ...
              <a href="mailto:parthmangukiya@shiveshwartextiles.com" className="text-[#faf8f4] hover:text-[#d4a96a] transition font-mono">
          ...
          {/* Phone */}
          <div className="flex gap-4 items-start border-l border-[#b8924a]/30 pl-4 py-1">
            <Phone className="h-5 w-5 text-[#d4a96a] shrink-0" />
            <div className="space-y-1">
              <span className="text-white/45 uppercase text-[10px] block">Parth Mangukiya (Export Coordinator)</span>
              <a href="tel:+919316189146" className="text-[#faf8f4] hover:text-[#d4a96a] transition font-mono">
                +91 93161 89146
              </a>
            </div>
          </div>
          ...
          <div className="flex gap-4 items-start border-l border-[#b8924a]/30 pl-4 py-1">
            ...
            <div className="space-y-1">
              <span className="text-white/45 uppercase text-[10px] block">Operating Hours</span>
              <span className="text-[#faf8f4]">
                Monday – Saturday: 09:00 – 19:00 IST (UTC+5:30)
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="pt-6 border-t border-[#b8924a]/10 flex flex-col sm:flex-row items-center gap-4 text-xs">
          <div className="flex items-center gap-2 text-white/50">
            <ShieldCheck className="h-4.5 w-4.5 text-[#d4a96a]" />
            <span>GDPR Compliance Secure Channel</span>
          </div>
      ...
      <div className="border border-[#b8924a]/18 bg-[#171513] p-6 md:p-10">
        <form className="space-y-5" onSubmit={handleSubmit}>
          ...
          <div className="grid gap-5 sm:grid-cols-2">
            {/* Phone Number */}
            <label className="grid gap-2">
              <span className="text-[10px] uppercase tracking-wider text-[#d4a96a] font-mono">Phone Number *</span>
              <input
                type="tel"
                required
          ...
          <div className="border border-[#b8924a]/15 bg-[#121110] p-5 rounded-xs space-y-3">
            <span className="text-[10px] uppercase tracking-widest text-[#d4a96a] font-mono block">Inquiry Summary</span>
            {cartItems.length === 0 ? (
              <p className="text-xs text-white/50 italic">No fabrics selected yet.</p>
            ) : (
              <div className="space-y-3">
                <div className="divide-y divide-[#b8924a]/10 max-h-48 overflow-y-auto pr-1">
                  {cartItems.map((item) => (
                    <div key={item.id} className="py-2.5 flex items-center justify-between text-xs gap-4">
                      <div className="flex items-center gap-3">
                        <img src={item.image} alt={item.name} className="h-8 w-8 object-cover border border-[#b8924a]/15" />
                        <div>
                          <span className="text-white font-medium block">{item.name}</span>
                          <span className="text-[10px] text-white/50 font-mono uppercase">{item.gsm} {item.fabricType ? `| ${item.fabricType}` : ''}</span>
                        </div>
                      </div>
                      <span className="text-[#d4a96a] font-mono font-medium">{item.quantity} meters</span>
                    </div>
                  ))}
                </div>
                <button
                  type="button"
                  onClick={openDrawer}
                  className="text-[9px] uppercase tracking-wider text-[#d4a96a] hover:text-white transition font-mono underline"
                >
                  Modify Selected Fabrics / RFQ Quantities
                </button>
              </div>
            )}
          </div>
      ...
      {isSubmitted && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/85 backdrop-blur-md animate-fade-in">
          ...
          />
          <div className="relative w-full max-w-md bg-[#141211] border border-[#b8924a]/30 shadow-2xl p-8 text-center space-y-6 animate-scale-in z-10">
            ...
            <div className="space-y-2">
              <h3 className="font-serif text-2xl text-white uppercase tracking-wider">Inquiry Submitted Successfully</h3>
              <p className="text-xs text-[#d4a96a] uppercase tracking-widest font-mono">Our team will contact you shortly</p>
            </div>
            
            <p className="text-xs leading-relaxed text-white/70">
              Our B2B coordinators will review your requirements and get back to you with specs and logistics estimates within 1 business day.
            </p>
            
            <button
              onClick={() => setIsSubmitted(false)}
              className="bg-[#b8924a] hover:bg-[#d4a96a] text-[#0f0e0c] font-bold uppercase tracking-widest text-xs py-3 w-full transition cursor-pointer"
            >
              Close
            </button>
          </div>
